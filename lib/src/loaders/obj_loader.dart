...
class OBJLoader extends Loader {
  ...
  Future<Object3D> load(String url, {String baseUrl, String crossOrigin}) =>
      HttpRequest.request(((baseUrl != null) ? baseUrl : '') + url,
              responseType: "String")
          .then((req) {
  ...
  void _addFace(
      Geometry geometry, int face_offset, String a, String b, String c,
  ...
  _handle_face_line(
      Geometry geometry, int faceOffset, normals, List uvs, List<String> faces,
      [List<String> uvsLine = null, normals_inds = null]) {
    if (faces[3] == null) {
      _addFace(geometry, faceOffset, faces[0], faces[1], faces[2], normals,
          normals_inds);
      ...
      if (normals_inds != null && normals_inds.length > 0) {
        _addFace(geometry, faceOffset, faces[0], faces[1], faces[3], normals,
            [normals_inds[0], normals_inds[1], normals_inds[3]]);
        _addFace(geometry, faceOffset, faces[1], faces[2], faces[3], normals,
  ...
  ///
  /// A group of [Object3D] is return
  Future<Object3D> parse(String text,
      {String baseUrl: "", String crossOrigin: ''}) {
    ...
    // v float float float
    var vertex_pattern = new RegExp(
        r"v( +[\d|\.|\+|\-|e]+)( +[\d|\.|\+|\-|e]+)( +[\d|\.|\+|\-|e]+)");

    // vn float float float
    var normal_pattern = new RegExp(
        r"vn( +[\d|\.|\+|\-|e]+)( +[\d|\.|\+|\-|e]+)( +[\d|\.|\+|\-|e]+)");
    ...
    lines.forEach((line) {
      ...
      if (!(line.length == 0 || line.startsWith('#'))) {
        if ((result = vertex_pattern.firstMatch(line)) != null) {
          ...
          _handle_face_line(geometry, face_offset, normals, uvs,
          ...
          _handle_face_line(geometry, face_offset, normals, uvs,
              [result[2], result[5], result[8], result[11]], [], //uv
          ...
          if (_useMtl) {
            ...
            future.then((materialCreator) {
              controllerMtl.stream.listen((Mesh e) {
                if (e.material is MeshLambertMaterial &&
                    e.material.name.isNotEmpty) {
