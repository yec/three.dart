  ...
  zoomCamera() {
    if (_state == STATE.TOUCH_ZOOM) {
      ...
      (object as OrthographicCamera).left =
          _zoomFactor * left0 + (1 - _zoomFactor) * center0.x;
      (object as OrthographicCamera).right =
          _zoomFactor * right0 + (1 - _zoomFactor) * center0.x;
      (object as OrthographicCamera).top =
          _zoomFactor * top0 + (1 - _zoomFactor) * center0.y;
      (object as OrthographicCamera).bottom =
          _zoomFactor * bottom0 + (1 - _zoomFactor) * center0.y;
    } else {
  ...
  touchstart(TouchEvent event) {
    ...
    switch (event.touches.length) {
      case 1:
        ...
        _rotateStart = _rotateEnd = getMouseProjectionOnBall(
            event.touches[0].page.x, event.touches[0].page.y);
        break;
      case 2:
        _state = STATE.TOUCH_ZOOM;
        _zoomStart = _zoomEnd =
            getMouseOnScreen(event.touches[0].page.x, event.touches[0].page.y);
        break;
      case 3:
