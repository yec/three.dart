  ...
  SkinnedMesh(geometry, material, {this.useVertexTexture: true})
    ...
    if (geometry["bones"] != null) {
      ...
      if (useVertexTexture) {
        ...
        boneMatrices = new Float32List(boneTextureWidth *
            boneTextureHeight *
            4); // 4 floats per RGBA pixel
        boneTexture = new DataTexture(
            boneMatrices, boneTextureWidth, boneTextureHeight, RGBAFormat,
            type: FloatType);
  ...
  void pose() {
    ...
    if (geometry["skinVerticesA"] == null) {
      ...
      for (var i = 0; i < geometry.skinIndices.length; i++) {
        ...
        geometry["skinVerticesA"]
            .add(vertex.applyProjection(boneInverses[indexA]));

        vertex = new Vector3(orgVertex.x, orgVertex.y, orgVertex.z);
        geometry["skinVerticesA"]
            .add(vertex.applyProjection(boneInverses[indexB]));
        ...
        if (geometry.skinWeights[i].x + geometry.skinWeights[i].y != 1) {
          var len =
              (1.0 - (geometry.skinWeights[i].x + geometry.skinWeights[i].y)) *
                  0.5;
