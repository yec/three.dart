...
class JSONLoader extends Loader {
  ...
  _loadAjaxJSON(String url, LoadedCallback callback, String texturePath,
      {LoadProgressCallback callbackProgress: null}) {
      ...
      if (xhr.readyState == HttpRequest.DONE) {
        if (xhr.status == 200 || xhr.status == 0) {
          if (!xhr.responseText.isEmpty) {
            ...
            print(
                "THREE.JSONLoader: [$url] seems to be unreachable or file there is empty");
          }
        ...
        if (callbackProgress != null) {
          ...
          callbackProgress(
              {"total": length, "loaded": xhr.responseText.length});
        }
  ...
  _parseSkin(Map json, Geometry geometry) {
    ...
    if (json.containsKey("skinWeights")) {
      ...
      for (i = 0; i < l; i += 2) {
        ...
        geometry.skinWeights.add(new Vector4(
    ...
    if (json.containsKey("skinIndices")) {
      ...
      for (i = 0; i < l; i += 2) {
        ...
        geometry.skinIndices.add(new Vector4(
            a.toDouble(), b.toDouble(), c.toDouble(), d.toDouble()));
      }
    }
  ...
  _parseMorphing(Map json, Geometry geometry, num scale) {
    if (json.containsKey("morphTargets")) {
      ...
      for (i = 0; i < geometry.morphTargets.length; i++) {
        geometry.morphTargets[i] = new MorphTarget(
            name: json["morphTargets"][i]["name"], vertices: []);
    ...
    if (json.containsKey("morphColors")) {
      ...
      for (i = 0; i < geometry.morphColors.length; i++) {
        ...
        geometry.morphColors[i] = new MorphColors(
            name: json["morphColors"][i]["name"], colors: dstColors);
      }
    }
  }
}
